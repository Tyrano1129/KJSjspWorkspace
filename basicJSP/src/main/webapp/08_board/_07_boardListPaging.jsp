...
<h1>전체 게시글 갯수 <%=List.size() %></h1>
	...
	<%	
		...
		<a href="_07_boardListPaging.jsp?start=<%=startPage - 1%>&curNum=<%=startPage - onePageNumcnt%>">[이전]</a>
	<%} %>
	<% 
		for(int i = startPage; i <= onePageNumEnd; i+=1){
			%>
			
			<a href="_07_boardListPaging.jsp?start=<%=i%>&curNum=<%=startPage%>">[<%=i%>]</a>
			
		<%}
	if(onePageNumEnd < totalPageNum){%>
		<a href="_07_boardListPaging.jsp?start=<%=startPage + onePageNumcnt%>&curNum=<%=startPage + onePageNumcnt%>">[이후]</a>
	<%} %>
</body>
</html>